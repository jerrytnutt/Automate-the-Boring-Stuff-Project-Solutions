...
# With integer N and integer M, Start at row N and insert M blank rows into the spreadsheet. 

from openpyxl import load_workbook, Workbook
import sys

def enter_blank_row(N,M,name):
  ...
  if len(sys.argv) > 2:
    try:
      if int(sys.argv[1]) and int(sys.argv[2]) :
        enter_blank_row(sys.argv[1],sys.argv[2],sys.argv[3])
    except ValueError:
